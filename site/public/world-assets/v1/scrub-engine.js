/* ============================================================================
   VENDORED FILE — not linted or formatted by biome (see site/biome.json).

   Scroll engine adapted from KubeezMedia/kubeez-scroll-world-video (MIT).
   Source: assets/scrub-engine.js @ c95d13cc. Licence is declared at
   .claude-plugin/plugin.json:9; the repository ships no LICENSE file.

   dz-patch-set: 1
   Local changes are marked `// dz-patch:` and are hardening only — no behaviour
   redesign — so the next upstream sync stays a diff rather than a merge:
     P1  evict off-screen clips + URL.revokeObjectURL   (upstream never revokes)
     P2  park the rAF loop while the document is hidden (upstream re-arms forever)
     P3  bound the clip-fetch retry                     (upstream retries a 404 every frame)
     P4  eager + high-priority first still              (upstream lazy-loads the LCP image)
     P5  real alt text from section.stillAlt            (upstream hardcodes alt="")
   P6 (pre-JS track height, a CLS fix) needs no engine change — it is a
   `min-height` on .sw-track in site/src/world/world.scss.
   ========================================================================== */
...
function mountScrollWorld(container, config) {
  ...
  // dz-patch:P1: distance (in viewport-heights) past a segment's edge at which its clip is
  // torn down again. Must exceed the ±1.6 load window below or a segment would thrash.
  const EVICT = 2.5;
  ...
  SECTIONS.forEach((s, i) => {
    // dz-patch:P5: carry stillAlt through to the segment so the <img> gets real alt text.
    const dive = { kind: 'dive', si: i, clip: s.clip, clipM: s.clipMobile, still: s.still, accent: s.accent,
                   stillAlt: s.stillAlt || '', w: s.scroll || DIVE_W, linger: s.linger || 0 };
    SEGMENTS.push(dive);
    s._seg = dive;
    ...
    if (i < N - 1 && CONNECTORS[i]) {
      // dz-patch:P5: no stillAlt here on purpose — a connector borrows the NEXT scene's
      // still, which that scene already announces. Describing it twice is screen-reader
      // noise, so it stays decorative (alt="").
  ...
  SEGMENTS.forEach((s, i) => {
    ...
    img.alt = s.stillAlt || '';                 // dz-patch:P5 (upstream: always '')
    img.decoding = 'async';
    // dz-patch:P4: the first segment's still is on screen at load and is the LCP
    // candidate — upstream lazy-loads every still, which defers the first paint.
    ...
    s.tries = 0; s.failed = false;              // dz-patch:P3
  });
  ...
  function loadClip(s) {
    ...
    if (reduce || s.loading || s.failed || !s.clip) return;   // dz-patch:P3: `s.failed`
    s.loading = true;
    ...
    fetch(url).then(r => r.ok ? r.blob() : Promise.reject(new Error('404')))
      .then(blob => {
        ...
        // dz-patch:P3: upstream clears `loading` unconditionally, so a clip that 404s is
        // re-fetched from read() on every scroll frame, forever. Bound the retries and
        // then leave the segment on its still.
        s.tries = (s.tries || 0) + 1;
        if (s.tries >= 3) { s.failed = true; return; }
        s.loading = false;
      });
  }

  // dz-patch:P1: upstream loads every clip as a Blob and never releases it — nine clips
  // stay decoded for the life of the page, which blows past the iOS concurrent-video
  // budget and leaks every object URL. Tear a far-away clip back down to its still; read()
  // will reload it if the visitor scrolls back.
  function unloadClip(s) {
    if (!s.video) return;
    const url = s.video.src;
    try { s.video.pause(); } catch (e) {}
    s.video.removeAttribute('src');
    try { s.video.load(); } catch (e) {}   // drops the decoder's hold before we revoke
    s.video.remove();
    if (url && url.indexOf('blob:') === 0) URL.revokeObjectURL(url);
    s.video = null; s.hasClip = false; s.ready = false; s.loading = false; s.cur = 0;
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      // dz-patch:P1: outside the wider evict band, release the clip again.
      else if (y < s.start - EVICT * vh || y > s.end + EVICT * vh) unloadClip(s);
  ...
  // dz-patch:P2: upstream re-arms the loop unconditionally, so it keeps running for the
  // life of the page — including on a hidden tab, where there is nothing to scrub. Park it
  // on visibilitychange and restart on return. (rAF ids are always non-zero, so 0 is a
  ...
  layout();
  startRaf();                              // dz-patch:P2 (upstream: requestAnimationFrame(raf))
